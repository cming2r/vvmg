...
export default function AdminPicHealth() {
      ...
      try {
        ...
        if (user?.email === ALLOWED_EMAIL) {
          const { data, error } = await supabase
            .from('health_scan')
            .select('id, image_url, ocr_result, device_id, country_code, add_from, created_at')
            .order('created_at', { ascending: false });

          if (error) {
            setError(error.message);
          } else {
            setScans((data as HealthScan[]) || []);
          }
        }
      } catch (err) {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-2xl font-bold text-gray-900">PicHealth - Health Scans</h1>
              <div className="flex items-center gap-4">
                <a href="/admin/pichealth/summary" className="text-sm text-blue-600 hover:text-blue-500">
                  View Summaries
                </a>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {paged.map((scan, idx) => (
                        <tr key={scan.id} className="hover:bg-gray-50">
                          ...
                          <td className="p-0 w-32 h-32">
                            {scan.image_url ? (
                              <a href={scan.image_url} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
                                <img
                                  src={scan.image_url}
                                  alt="scan"
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900 max-w-md">
                            <pre className="whitespace-pre-wrap text-xs bg-gray-50 p-2 rounded max-h-32 overflow-y-auto">
                              {JSON.stringify(
                                Object.fromEntries(
                                  Object.entries(scan.ocr_result).filter(([k]) => k !== 'rawText')
                                ),
                                null,
                                2
                              )}
